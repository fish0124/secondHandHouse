// topDiv部分 切换
$(function() {
	var img_s = $("#overview .imgplayer").offset().top;
    // console.log(img_s);
...
// “关注房源”点击效果
$(function() {
	$("#overview p span.like").click(function() {
		$(this).addClass('hide');
		$(this).siblings('span').removeClass('hide');
	});
	$("#overview p span.liked").click(function() {
		$(this).addClass('hide');
		$(this).siblings('span').removeClass('hide');
	});
});
...
// 留言区域 点击效果
$(function() {
	$("#overview .messages .topArea").on("click",function() {
		// 左侧小箭头的转换
		$(this).children('i:last-child').toggleClass('glyphicon-chevron-left');
		$(this).children('i:last-child').toggleClass('glyphicon-chevron-down');

		// 留言区域的切换
		var txtArea = $("#overview .messages .txtArea");
		if(txtArea.css("opacity") == 0) {
			txtArea.animate({"opacity": 1},600);
		}else{
			txtArea.animate({"opacity": 0},600);
		}
	});
});